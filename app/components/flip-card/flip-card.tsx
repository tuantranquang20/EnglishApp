...
const FlipCard = React.forwardRef((props, ref) => {
  ...
  function Flip(direction) {
    ...
    let toValue = cardFaceRef.current ? direction * width : direction * width + lastScrollX.current
    if (flippedValue.current != null && cardFaceRef.current) {
      toValue = direction * width + lastScrollX.current
    }
    isAnimating.current = true
  ...
  const renderFront = () => {
    return (
      ...
      >
        <View pointerEvents={cardFace ? "auto" : "none"}>{props.children[0]}</View>
      </Animated.View>
    )
  }
  ...
  const renderBack = () => {
    return (
      ...
      >
        <View pointerEvents={cardFace ? "none" : "auto"}>{props.children[1]}</View>
      </Animated.View>
    )
  }
