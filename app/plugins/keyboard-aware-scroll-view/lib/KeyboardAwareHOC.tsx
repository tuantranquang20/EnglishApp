...
function KeyboardAwareHOC(ScrollableComponent: any, userOptions: any = {}) {
    const hocOptions: any = {
        ...
        implements KeyboardAwareInterface
    {
        ...
        keyboardWillShowEvent: any;
        keyboardWillHideEvent: any;
        position: ContentOffset;
        defaultResetScrollToCoords: any;
        mountedComponent: any;
        handleOnScroll: any;
        state: KeyboardAwareHOCState;
        static displayName = `KeyboardAware${getDisplayName(
            ScrollableComponent,
        )}`;
        ...
        componentDidMount() {
            ...
            supportedKeyboardEvents.forEach((eventName: string) => {
                const callbackName: any =
                    keyboardEventToCallbackName(eventName);
                if ((this.props as any)[callbackName]) {
                    this.callbacks[eventName] = Keyboard.addListener(
                        eventName as any,
                        (this.props as any)[callbackName],
                    );
                }
            });
        }
        ...
        componentDidUpdate(prevProps: KeyboardAwareHOCProps) {
            if (
                this.props.viewIsInsideTabBar !== prevProps.viewIsInsideTabBar
            ) {
        ...
        _defaultGetScrollPosition = (
            ...
            return {
                ...
                y: Math.max(
                    0,
                    childLayout.y - parentLayout.y + contentOffset.y,
                ),
                animated: true,
            };
        };
        ...
        _measureElement = (element: any): Promise<ElementLayout> => {
            const node: any = findNodeHandle(element);
        ...
        // Keyboard actions
        _updateKeyboardSpace = (frames: any) => {
        ...
        _handleOnScroll = (
            e: any & { nativeEvent: { contentOffset: number } },
        ) => {
        ...
        _handleRef = (ref: any) => {
            this._rnkasv_keyboardView = ref
                ? hocOptions.extractNativeRef(ref)
                : ref;
        ...
        render() {
            const { enableOnAndroid, contentContainerStyle, onScroll } =
                this.props;
            let newContentContainerStyle;
            let arr: any[] = [];
            if (Platform.OS === 'android' && enableOnAndroid) {
                newContentContainerStyle = arr
            ...
            return (
                <ScrollableComponent
                    ...
                    update={this.update}
                    onScroll={(Animated as any).forkEvent(
                        onScroll,
...
const listenToKeyboardEvents: any = (configOrComp: any) => {
